...
| Finding | Disposition | Change or follow-up |
|---|---|---|
| 1 | Fixed | Every cancellation cascade released the key only when `KeyGate::key_holder` named the canceled run (the run-level cascade helper was later replaced by the task-level durable cascade in finalization, F-029). Scheduled and Ready runs release nothing and emit no warning. A dispatch test captures WARN output and proves the spurious "key not held" line is gone. |
