#!/usr/bin/env python3
"""Independent projection v5 vector; preserves all earlier evidence."""
import hashlib
import json
import struct
...
p = json.loads((root / 'projection-v4-vector.json').read_text())['projection']
p['version'] = p['metadata']['schema_version'] = 5
for name in ['dispatch_sequences', 'administrative_wakes', 'administrative_pending']:
    p[name] = []
(root / 'projection-v5-vector.json').write_text(json.dumps({
    'projection': p,
    'sha256': hashlib.sha256(b'AQ-CONT-1\0projection\0v5\0' + tree(p)).hexdigest(),
