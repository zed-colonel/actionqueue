//! DAG dependencies survive snapshot-based recovery.
//!
//! Proves that dependency declarations are persisted in snapshots and correctly
//! reconstructed during snapshot-based bootstrap, maintaining DAG ordering
//! guarantees even when the WAL events that originally declared the dependencies
//! are no longer replayed (they precede the snapshot).
...
mod wf {
    ...
    fn engine_config(data_dir: &std::path::Path, snapshot_threshold: Option<u64>) -> RuntimeConfig {
        RuntimeConfig {
            data_dir: data_dir.to_path_buf(),
            snapshot_event_threshold: snapshot_threshold,
    ...
    #[tokio::test]
    async fn dag_dependencies_survive_snapshot_recovery() {
        let data_dir = super::support::unique_data_dir("wf-dag-snapshot-recovery");
        let execution_log: Arc<Mutex<Vec<String>>> = Arc::new(Mutex::new(vec![]));

        let step1_id: TaskId = "07010101-0001-0001-0001-000000000001".parse().expect("valid uuid");
        let step2_id: TaskId = "07010101-0001-0001-0001-000000000002".parse().expect("valid uuid");
        let step3_id: TaskId = "07010101-0001-0001-0001-000000000003".parse().expect("valid uuid");
        ...
        // step2 depends on step1; step3 depends on step2.
        {
            ...
            let _ = authority
                ...
                .expect("declare step3→step2");
        }

        // Phase 3: run step1 to completion, then force a snapshot.
        {
            let log = Arc::clone(&execution_log);
        ...
        // Phase 4: restart from snapshot + WAL tail and verify dependency
        // declarations survived snapshot-based recovery.
        {
            let recovery = load_projection_from_storage(&data_dir).expect("recovery must succeed");
            assert!(recovery.snapshot_loaded, "snapshot must be loaded (threshold was set to 1)");

            // Verify dependency declarations exist in the projection.
            let dep_count = recovery.projection.dependency_declarations().count();
            assert_eq!(
                dep_count, 2,
                "exactly 2 dependency declarations must survive snapshot-based recovery \
                 (step2→step1 and step3→step2), got {dep_count}"
            );

            // Verify all three tasks and runs survived.
            assert_eq!(recovery.projection.task_count(), 3);
            for (task_id, name) in [(step1_id, "step1"), (step2_id, "step2"), (step3_id, "step3")] {
                let run_ids = recovery.projection.run_ids_for_task(task_id);
                assert_eq!(run_ids.len(), 1, "{name} must have one run after snapshot recovery");
                assert_eq!(
                    recovery.projection.get_run_state(&run_ids[0]),
                    Some(&RunState::Completed),
                    "{name} run must be Completed after snapshot recovery"
                );
            }
        }

        // Phase 5: verify execution order from the tracking log.
