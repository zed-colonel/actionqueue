...
/// Proves that 6 diverse mutations through a single authority session
...
fn multi_mutation_session_produces_consistent_projection_after_each_step() {
    ...
    let request = actionqueue_core::admission::EnsureTaskRequest::for_task(spec_a, vec![]).unwrap();
    let cmd1 = MutationCommand::AdmissionCommit(admission_support::command(request, seq1, seq1));
    let _ = authority
    ...
    let request = actionqueue_core::admission::EnsureTaskRequest::for_task(spec_b, vec![]).unwrap();
    let cmd2 = MutationCommand::AdmissionCommit(admission_support::command(request, seq2, seq2));
    let _ = authority
        .submit_command(cmd2, DurabilityPolicy::Immediate)
        .expect("task create should succeed");
    assert_eq!(authority.projection().task_count(), 2);

    // Both Once runs were committed with their task admissions.
    let run_a_id = authority.projection().run_ids_for_task(task_a_id)[0];
    assert_eq!(authority.projection().run_count(), 2);
    assert_eq!(authority.projection().get_run_state(&run_a_id).copied(), Some(RunState::Scheduled));

    // Mutation 3: Promote run A from Scheduled to Ready.
    let seq3 = next_seq(&authority);
    let cmd3 = MutationCommand::RunStateTransition(RunStateTransitionCommand::new(
        seq3,
        run_a_id,
        RunState::Scheduled,
        RunState::Ready,
    ...
    // Mutation 6: Cancel task B.
    let seq6 = next_seq(&authority);
    let cmd6 = MutationCommand::TaskCancel(TaskCancelCommand::new(seq6, task_b_id, seq6));
    let _ =
        authority.submit_command(cmd6, DurabilityPolicy::Immediate).expect("cancel should succeed");
    ...
    assert_eq!(authority.projection().latest_sequence(), seq6);
    assert_eq!(authority.projection().task_count(), 2);
    assert_eq!(authority.projection().run_count(), 2);
    assert_eq!(authority.projection().get_run_state(&run_a_id).copied(), Some(RunState::Ready));
    ...
    assert_eq!(replayed.run_count(), 2);
    ...
    assert_eq!(replayed.latest_sequence(), seq6);
