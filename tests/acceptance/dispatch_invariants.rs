...
proptest! {
    ...
    fn prop_once_dispatch_never_exceeds_bound(
        ...
        rt.block_on(async {
            ...
            let clock = MockClock::new(1_000_000);
            let engine = ActionQueueEngine::new(config, AlwaysRetryHandler);
            let mut boot = engine.bootstrap_with_clock(clock).expect("bootstrap").with_host(crate::admission_support::host_support::host(actionqueue_core::control::ControlScope::SingleTenant));
